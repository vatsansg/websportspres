...
| E4 | Partial-failure data integrity — a storage failure after the DB write does not leave an orphaned/ghost event | Pass, by design (not live-forced — see QA doc TC-14) | `routes.js` wraps the DB insert in `withTransaction`; if the subsequent `createEventStorageStructure` call throws, the `catch` block deletes the `events` row (cascades to `event_tables` via `ON DELETE CASCADE`) and attempts `deleteEventStorageStructure` before rethrowing a generic 500. **Independent architect review caught two related gaps, both fixed same day**: (1) the DB-row rollback and the storage cleanup were silently swallowed on failure (`.catch(() => {})` with no logging) — a double-failure (storage create fails, then rollback also fails) would leave orphaned state with zero trace; now both are logged via `console.error` if they themselves fail. (2) a TOCTOU race: the pre-check `SELECT` for duplicate Event ID runs outside the transaction, so two concurrent requests for the same new ID could both pass it and race on the `INSERT`, with the loser getting an ungraceful 500 instead of 409 — now caught via MySQL's `ER_DUP_ENTRY` error code and converted to a clean 409. |
...
| E6 | Credential-guessable/abuse-prone endpoints have rate-limiting protection | Pass | **Independent architect review flagged**: unlike `/api/auth/login`, `POST /api/events` had no rate limit, letting any authenticated user (including Normal User) loop-create events and exhaust the ID namespace or spam storage/DB. Fixed: added `express-rate-limit` (20/15min), keyed by session username rather than IP (abuse here is per-account, not per-network-address). |
