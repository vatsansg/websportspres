...
| E8 | Concurrent writes to the same event's change log cannot silently lose an entry | Pass | `appendChangeLogEntry()` (`logging/assetChangeLog.js`) uses ETag-conditional writes with retry (up to 5 attempts) - a lost-update race between two concurrent requests appending to the same event's `_ledassetschangelog.csv` is detected (412 Precondition Failed) and retried rather than silently overwriting the other request's entry. Not the case for `sponsorsequence.csv` writes (D-02 below). |
