...
| TC-11 | App Service → MySQL connectivity via Managed Identity (no password) | TC-05, TC-09 passed | 1. Grant Directory Readers to `uami-mysql-sportspres-assetmgmt` (blocked — see below) 2. Run `bootstrap-mysql-aad.mjs` to create AAD-mapped MySQL users for both App Service identities 3. Deploy app, hit `/api/health/db` | `{ ok: true }`, connecting as `app-sportspres-assetmgmt` / `app-sportspres-assetmgmt-dev` MySQL users with no password/connection string anywhere | **Blocked**, with real confirming evidence from the live dev slot: `{"ok":false,"error":"Access denied for user 'app-sportspres-assetmgmt-dev'@'...' (using password: YES)"}`. This proves the App Service genuinely is attempting the connection using its own Managed Identity's AAD token (not a static password — `password: YES` here refers to mysql2's generic auth-attempt flag, not a real password) — it fails only because the MySQL AAD user doesn't exist yet, exactly as expected. No other misconfiguration is masked behind this error. | Blocked | Needs a tenant directory admin (Privileged Role Administrator/Global Administrator) to grant Directory Readers to principal `5514ec52-fbe8-4a86-b771-566fac4d67b8` (see handoff notes). User has chosen this resolution path over disabling `aad_auth_validate_oids_in_tenant`. **Important discovery**: this failure originally crashed the *entire app* (not just this one health check) because `seedSuperAdmin()` ran unconditionally at startup and its failure was fatal — fixed in this step (see D-05) so the app now degrades gracefully (health/config/storage/frontend all still serve) while this specific blocker is outstanding. |
