...
- **Still open:**
  ...
  7. **User reported the popup gets stuck on "Completing sign-in..." forever.** First attempted fix (skip Angular bootstrap in the popup, on the theory that the opener directly polls `popupWindow.location.href`) was based on an **incorrect assumption about the mechanism** — deployed it, user retried, still stuck. Read the actual installed `@azure/msal-browser` v5.22.0 source (`node_modules/@azure/msal-browser/lib/redirect-bridge/`) rather than guessing again: this version does **not** use opener-side polling at all. The redirect URI page itself must call `broadcastResponseToMainFrame()` (from the `@azure/msal-browser/redirect-bridge` subpath export), which parses the response out of the URL, broadcasts it to the opener over a `BroadcastChannel`, and closes the popup — nothing was ever calling this, in either the old `auth-callback` component or the first (wrong) fix. **Real fix**, `client/src/main.ts`: when the URL is `/auth/callback` with an auth response in the hash/query, dynamically import and call `broadcastResponseToMainFrame()` instead of bootstrapping Angular at all (matches the function's own requirement that nothing touch the URL first).
  8. **Verified the real fix executes correctly**, 2026-09-16, via browser automation: manually drove the Microsoft account-picker flow to `/auth/callback` in a tracked tab (a hand-built authorize URL, deliberately without `state`/PKCE params, so it can't fully succeed — that requires the real `loginPopup()`-constructed request). Confirmed via the resulting tab: page title changed to **"Microsoft Authentication"** — literally the first line of `broadcastResponseToMainFrame()` — and the URL's hash was cleared (`history.replaceState`), both proving the new code path runs. Got the documented `BrowserAuthError: empty_response` (correctly, since my manual URL had no `state` param for the function to find), not a crash or silent hang.
  9. **User retried after item 8's fix — this time got a real, different error: `post_request_failed` / `TypeError: Failed to fetch`.** This meant the popup mechanism now fully works (the flow reached MSAL's authorization-code-for-token exchange, a step that never happened before) — the new failure was the browser blocking that exchange's `fetch()` call. Root cause: `helmet()`'s default Content-Security-Policy restricts `connect-src` to `'self'` only, silently blocking any `fetch()`/XHR to `https://login.microsoftonline.com` from our own page (CSP blocks manifest identically to network failures — a generic `TypeError: Failed to fetch`, no explanation). Fixed in `server/src/app.js`: explicit CSP config adding `https://login.microsoftonline.com` to `connect-src` only (kept every other helmet default). Verified the deployed header directly (`curl -D -`) shows the corrected `connect-src` directive live on the `dev` slot. **Asked the user to retry sign-in again.**
