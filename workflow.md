...
- **What was built:**
  ...
  - **Azure resources** (subscription `b239d404-c0c0-4098-a287-f9983f71d56b`, RG `rgsportspresentationsource`, region `southeastasia`):
    ...
    - MySQL Flexible Server `mysql-sportspres-assetmgmt` (General Purpose, `Standard_D2ds_v4`, MySQL 8.0.21), firewall-based network access (per your choice over VNet/private endpoint) with the standard "allow Azure services" rule plus the developer's own IP for admin scripts
...
- **Deviations from plan (if any):**
  1. **LED configuration schema placement.** BRD Section 5 lists Inner/Outer/Main LED as columns on the `Events` table. Sections 3.1, 6, 9, 10, 11.1, and — decisively — Section 25.4's export JSON spec ("each `tables` entry contains the table number **and** the Inner/Outer/Main LED booleans configured for **that table**") all show LED config is actually per-**table**. Built `event_tables` (one row per table, with LED booleans + per-LED-type resolution) instead of putting LED columns on `events`. **Please confirm this reading is correct** — it's a foundational schema decision for every step that follows.
...
- **Blockers — need your input / WTT IT before this step can be marked fully Complete:**
  1. **MySQL AAD auth for the App Service itself is not yet working.** Azure requires the MySQL server's helper identity (`uami-mysql-sportspres-assetmgmt`, principal `5514ec52-fbe8-4a86-b771-566fac4d67b8`) to hold the **Directory Readers** role in Entra ID before `CREATE AADUSER` can run for the App Service's own managed identities. I don't have permission to grant this (confirmed: Microsoft Graph rejected the attempt as insufficient privilege). You chose to resolve this by having a tenant directory admin grant the role, rather than disabling the `aad_auth_validate_oids_in_tenant` safety check. **Action needed:** ask whoever has Privileged Role Administrator/Global Administrator in the WTT tenant to: Entra ID → Roles and administrators → search "Directory Readers" → Add assignments → search for `uami-mysql-sportspres-assetmgmt` → Add. Once done, tell me and I'll run `server/scripts/bootstrap-mysql-aad.mjs` to finish wiring the App Service's MySQL access, and Step 1's Managed-Identity-to-MySQL requirement will be fully provable end-to-end (currently proven only via the developer's own identity, which has broader Owner-level access — see QA doc D-03/TC-06).
  2. **Azure AD app registration** for Administrator/Normal User login has not been completed yet. Numbered instructions were handed to you for WTT IT in this step's handoff — waiting on the resulting Tenant ID/Client ID being set as App Service Application Settings (`AZURE_AD_TENANT_ID`, `AZURE_AD_CLIENT_ID`) on `app-sportspres-assetmgmt`. Until then, `/api/config` correctly reports Azure AD as unconfigured and the "Sign In With Microsoft" button is inert rather than broken.
  3. **`ralph-loop` polish pass not yet run.** The plan calls for a second `wtt-brand` + `ralph-loop` pass after the feature works, purely for visual/UX fine-tuning. Have not run this yet — flagging so it isn't silently skipped. Recommend running it once you've had a chance to look at the login/change-password screens, in case you want to redirect the visual direction first.
  4. **Not yet deployed to the live App Service.** Everything above has been built and smoke-tested **locally** (using the developer's own `az login` session in place of Managed Identity) plus verified against Azure resource state directly via `az` CLI. The actual code has not yet been zip-deployed to `app-sportspres-assetmgmt`/`dev` — doing that next, before hand-off is final.
