...
- **Still open:**
  ...
  7. **Real bug found and fixed 2026-09-16: user reported the popup gets stuck on "Completing sign-in..." forever.** Root cause: `loginPopup()` works by having the *opener* window poll the popup's own `location.href` once Azure AD redirects it back to our origin, reading the auth response directly out of the URL fragment (`#id_token=...`). My `auth-callback` route let Angular bootstrap and run its Router's initial navigation inside that popup first — which strips/normalizes the URL fragment before the opener ever reads it, so the opener's `loginPopup()` call just sits there until its own timeout fires (matching the earlier `BrowserAuthError: timed_out`) while the popup shows my static placeholder text forever. Fixed in `client/src/main.ts`: detect "running in a popup with an unprocessed auth response in the URL hash" *before* calling `bootstrapApplication()`, and skip bootstrapping Angular entirely in that case, leaving the URL completely untouched for the opener to read (falls back to a plain-HTML "Completing sign-in..." in `index.html`, not Angular-rendered). Rebuilt, redeployed to `dev`, confirmed `/api/health` still ok. **Asked the user to retry the real sign-in.**
