...
- **Still open:**
  1. **Azure AD app registration** for Administrator/Normal User login has not been completed yet. Numbered instructions were handed to the user for WTT IT earlier — waiting on the resulting Tenant ID/Client ID being set as App Service Application Settings (`AZURE_AD_TENANT_ID`, `AZURE_AD_CLIENT_ID`) on `app-sportspres-assetmgmt`. Until then, `/api/config` correctly reports Azure AD as unconfigured and the "Sign In With Microsoft" button is inert rather than broken.
  2. **`ralph-loop` polish pass not yet run.** The plan calls for a second `wtt-brand` + `ralph-loop` pass after the feature works, purely for visual/UX fine-tuning. Have not run this yet — flagging so it isn't silently skipped. Recommend running it once the user has had a chance to look at the login/change-password screens, in case they want to redirect the visual direction first.
  3. **MySQL major version upgrade (8.0 → 8.4).** Azure Portal flagged that MySQL 8.0 loses standard support 2026-12-31 (extended-support charges start 2027-01-01). `az mysql flexible-server upgrade --version 8.4` is available as a supported in-place operation. Recommended to do this now, while there's no real event data yet (lowest-risk time) — **awaiting user decision on timing**, since it causes a brief restart/downtime.
