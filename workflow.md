...
- **Still open:**
  1. **Redirect URI platform type.** The registered redirect URI was added under the "Web" platform; needs to be under "Single-page application" instead (required for the public-client/PKCE flow this app uses — no client secret). Also, only the production callback URL was added; the dev slot's (`https://app-sportspres-assetmgmt-dev.azurewebsites.net/auth/callback`) hit an "invalid URL" error when adding it — instructions given to redo both under the SPA platform in one go. **Waiting on user to complete this in the portal.**
  2. **App Roles (Administrator, Normal User) not yet confirmed created/assigned.** Instructions given to create both roles and assign the user's own account to `Administrator` for initial testing. **Waiting on user to complete this in the portal** — without it, every AAD sign-in will be rejected with "no recognised app role assigned."
  ...
  3. **MySQL major version upgrade (8.0 → 8.4) — DONE 2026-09-16.** User confirmed doing this now. `az mysql flexible-server upgrade --version 8.4` completed successfully (`fullVersion: 8.4.9`, `state: Ready`). Re-verified `/api/health/db` still returns `{"ok":true}` post-upgrade.
